...
#6
    def get_unique_collection(self):
        return set(genre for movie in  self.lst for genre in (movie.get('genre_ids', [])or []))
#8
...
#10
    def get_data_and_change_copy(self):
        return self.lst.copy() and self.lst[0].update({'genre_ids': [22]}) or self.lst.copy()
#7
    ...
    def get_collection_of_structures(self):
        for movie in self.lst:
            mark = {
                    ...
                    'Last day in cinema': (datetime.strptime(movie['release_date'], '%Y-%m-%d') + timedelta(weeks = 8, days = 4)).strftime('%Y-%m-%d')
                    }
            self.collection.append(mark)
        self.collection.sort(key=lambda movie: (movie['Score'], movie['Popularity']))
...
#9
    def get_titles_grouped(self):
        return sorted(self.lst, key=lambda x: x.get('genre_ids', []))
...
#3
#pprint.pprint(test.index_3_to_19())
#4
#pprint.pprint(test.popularity())
#5
#pprint.pprint(test.names_titles_keywords(user_keywords))
#6
